...
def process_scan( path ):
    ...
    no_ocr_path = "%s.no_ocr.pdf"%os.path.join(base_path, filename, filename)
    ...
    try:
        ...
        if p.returncode != 0:
            log.error( "img2pdf exiting with %d." % p.returncode )
            return
    ...
    log.info( "PDF processing for % completed. Cleaning up files" )
